...
fn process_composer_version(
    ...
    requires.insert("heroku-sys/composer".to_string(), "*".to_string());
...
fn process_stability_flags(
    ...
    extract_platform_links_with_heroku_sys(stability_flags)
        .unwrap_or_default()
        .iter()
        .for_each(|(package_name, numeric_stability)| {
            additional_requires.insert(
                package_name.clone(),
...
pub(crate) fn extract_from_lock(
    ...
    (
        PlatformJsonGeneratorConfigNeedsBetterName,
        HashSet<PlatformExtractorNotice>,
    ),
    PlatformExtractorError,
> {
    ...
    Ok((
        PlatformJsonGeneratorConfigNeedsBetterName {
            ...
            require: Some(composer_requires.0),
            require_dev: None,
            stability_flags: Some(lock.stability_flags.deref().clone()),
        },
        composer_requires.1,
    ))
}
...
pub(crate) fn generate_platform_json(
    input: PlatformJsonGeneratorConfigNeedsBetterName,
    lock: &ComposerLock,
    stack: &str,
    installer_path: &Path,
    ...
    let stack_provide = (
        format!(
            "heroku-sys/{}",
            String::from(stack_captures.get(1).unwrap().as_str())
        ),
        format!(
            "{}.{}",
    ...
    // add requirements from caller
    requires.extend(input.require.unwrap_or_default());
    dev_requires.extend(input.require_dev.unwrap_or_default());
    ...
    // web servers
    requires.insert("heroku-sys/apache".to_string(), "^2.4.10".to_string());
    requires.insert("heroku-sys/nginx".to_string(), "^1.8.0".to_string());
    ...
    let mut dev_metapaks = Vec::new();

    // TODO WIP: this should be supplied by the caller
    for (is_dev, platform, packages, requires, metapaks) in [
        (
            false,
            &lock.platform,
            &lock.packages,
        ...
        if let Some(root_package) = extract_root_requirements(
            ...
            format!("dev-{}", lock.content_hash),
        ) {
            // we use this later to warn if no "php" requirement
            runtime_require_in_root |= !is_dev
                && has_runtime_requirement(
                    &root_package.package.require.clone().unwrap_or_default(),
                );

            // FIXME: this inserts require stability flags into require-dev
            requires.extend(process_stability_flags(
                &input.stability_flags.clone().unwrap_or_default(),
            ));

            metapaks.push(root_package);
        }

        // then, build packages for all regular requires
        metapaks.extend(packages.iter().filter_map(package_with_only_platform_links));
    }

    // TODO WIP: end

    // for given packages, scan each for runtime requirement, add a require(-dev) for each, and import all into a repo
    for (requires, marker, metapaks) in [
        (&mut requires, &mut seen_runtime_requirement, metapaks),
        (
            &mut dev_requires,
        ...
        if metapaks.is_empty().not() {
            for package in &metapaks {
                // check if there is a runtime requirement
                *marker |= package
                    .package
                    .require
                    .as_ref()
                    ...
                    .is_some();
                // TODO: check requires against stability flags
    ...
    // TODO refactor: should the caller be responsible for this somehow? maybe you don't want "php" required, e.g. for web server installs
    if !seen_runtime_requirement {
        ...
        // TODO refactor: have this be figured out by the caller
...
mod tests {
    ...
    struct ComposerLockTestCaseConfig {
        ...
        expected_notices: Option<Vec<String>>, // TODO: can we use PlatformGeneratorNotice?
        expect_failure: Option<String>,        // TODO: can we use PlatformGeneratorError?
    ...
    fn make_platform_json_with_fixtures() {
        ...
        fs::read_dir(&Path::new("tests/fixtures/platform/generator"))
            ...
            .for_each(|case| {
                ...
                let (generator_input, extractor_notices) = match generator_input
                {
                    ...
                    Err(e) => {
                        assert!(
                            ...
                            "case {}: lock extraction failed, but config has no expect_failure type specified",
                            case.name.as_ref().unwrap()
                        );
                ...
                // NEXT: generate "platform.json" from the extracted config and packages list
